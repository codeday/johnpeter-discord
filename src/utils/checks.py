...
def requires_staff_role():
    ...
    async def predicate(ctx: Context):
        ...
        staff_id = loads(
            getenv("ROLES_STAFF", '["689215241996730417", "712062910897061979"]'))
...
def requires_gold_role():
    async def predicate(ctx: Context):
        gold_id = getenv("ROLE_GOLD")
        if [i for i in [str(role.id) for role in ctx.author.roles] if i == gold_id]:
            return True
        raise MissingAnyRole(gold_id)
    return check(predicate)
...
def requires_tournament_role():
    ...
    async def predicate(ctx: Context):
        ...
        staff_id = loads(
            getenv("ROLES_TOURNAMENT", '["693206115349037069", "712062910897061979"]'))
        if [i for i in [str(role.id) for role in ctx.author.roles] if i in staff_id]:
            return True
        raise MissingAnyRole(staff_id)
    return check(predicate)
